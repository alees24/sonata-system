...
// Is the new write contiguously above from the previous write?
...
// qualification with the burst length may be required.
...
// By writing reordering a pair of words writes we can coalesce writes to descending addresses;
// to achieve longer burst writes in this case would require a LIFO implementation since the
// HyperRAM accepts only ascending bursts.
...
//
// TODO: Experimentally shorten the length of the write bursts to reduce read latency.
// wire burst_max = &burst_len_m1[Log2BurstLen-2-ABIT:0];
...
always_ff @(posedge clk_i) begin
  if (!stalled) begin
    // Address and burst length tracking.
    if (wr_start) begin
      // Current properties of burst.
...
// immediately and its predecessor in the ensuing (stall-free) cycle.
wire dfifo_wr_ena = flush_write | wr_descending | &{wr_stored, wr_storing, !addr_repeated};
// We must stall the sender if we cannot proceed with a data write.
...
// the data bytes until we have a complete word.
always_ff @(posedge clk_i) begin
...
assign dfifo_wr_strb_o = contig_below ? dfifo_wr_strb_i : strb_stored;
assign dfifo_wr_din_o  = contig_below ? dfifo_wr_din_i  : data_stored;

// Modified command traffic to the HyperRAM controller.
assign cmd_req_o = flush_write | (rd_req & ~addr_collision);
assign cmd_mem_addr_o = (flush_write & !contig_below) ? base_addr_stored : cmd_mem_addr_i;
...
assign cmd_wready_o = ~|{rd_req & flush_write,  // Must complete the write before we can read.
                         wr_req & stalled};     // Cannot accept more write data when stalled.
